...
.run(function (Sound,Device,Friends,Focus,Board,$filter,Task,socket,Meeting,Chat,$ionicScrollDelegate,Auth,$timeout,Team) {
	...
	socket.on('msg sent',function (data) {
      ...
      //   };
      // }
      // Chat.record[friendIndex].msg.push({
    ...
    socket.on('friend list',function (data) {
        ...
        for (i in data) {
          ...
          Friends.list.push({
            ...
            friendDB:window.low(window.path.join(Device.dataPath,'friend-'+data[i].name+'.json')),
            unread:0
          });
          Chat.record[i].msg=Friends.list[i].friendDB('chats').value();
        }
        ...
        for (i in Chat.unread)  {
          ...
          Friends.list[friendIndex].friendDB('chats').push(savemsg);
    ...
    socket.on('new msg from friend',function(data){
      ...
      var friendIndex=_.findIndex(Friends.list,{name:data.from})
        ...
        // Chat.record[friendIndex].msg.push(savemsg);  
        Friends.list[friendIndex].friendDB('chats').push(savemsg);
        if (!Device.focus || friendIndex!=Chat.currentIndex) {
          Sound.newmsg.play();
